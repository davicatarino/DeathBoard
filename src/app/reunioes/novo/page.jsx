...
export default function NovoReuniaoPage() {
  ...
  async function handleSubmit(e) {
    ...
    if (res.ok) {
      ...
      toast.error((await res.json()).message || "Erro ao agendar reunião");
    }
    setLoading(false);
  }
  ...
  return (
    <div className="max-w-lg mx-auto p-4">
      <h1 className="text-2xl font-bold mb-4 text-gray-100">Nova Reunião</h1>
      <form onSubmit={handleSubmit} className="space-y-4 bg-gray-800 p-6 rounded-lg shadow">
        ...
        <button type="submit" disabled={loading} className="w-full bg-blue-600 hover:bg-blue-700 text-white py-2 rounded disabled:opacity-50 font-semibold transition-all">
          {loading ? (
